...
class AttributeController extends Controller
{
    ...
    public function attrCreateForSubchildcategory($subchildcatid) {
      $data = Subchildcategory::findOrFail($subchildcatid);
    ...
    public function store(Request $request) {
      ...
      } elseif ($request->type == 'subchildcategory') {
        $in['attributable_type'] = 'App\Models\Subchildcategory';
      }
      ...
      if (request()->has('quantity_status')) {
        $in['quantity_status'] = 1;
      } else {
        $in['quantity_status'] = 0;
      }

      if (request()->has('image_status')) {
        $in['image_status'] = 1;
      } else {
        $in['image_status'] = 0;
      }
    ...
    public function manage(Request $request, $id) {
      ...
      if ($request->type == 'subchildcategory') {
        $data['data'] = Subchildcategory::find($id);
      }
    ...
    public function update(Request $request, $id) {
      ...
      if (request()->has('quantity_status')) {
        $attr->quantity_status = 1;
      } else {
        $attr->quantity_status = 0;
      }
