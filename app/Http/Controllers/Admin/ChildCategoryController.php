...
class ChildCategoryController extends Controller
{
   public function __construct()
    ...
    public function datatables()
    {
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('status', function(Childcategory $data) {
                                ...
                                return '<div class="action-list"><select class="child-select"><option data-val="1" value="'. route('admin-childcat-status',['id1' => $data->id, 'id2' => 1]).'" '.$s.'>Activated</option><option data-val="0" value="'. route('admin-childcat-status',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>Deactivated</option>/select></div>';
                            })
    ...
    public function store(Request $request)
    {
        //--- Validation Section
        $rules = [
            'slug' => 'unique:categories|unique:subcategories|unique:childcategories|unique:subchildcategories|regex:/^[a-zA-Z0-9\s-]+$/',
            'name' => 'unique:categories|unique:subcategories|unique:childcategories|unique:subchildcategories',
            'sequence' => 'unique:childcategories'
                 ];
        $customs = [
            'slug.unique' => 'This slug has already been taken.',
            'slug.regex' => 'Slug Must Not Have Any Special Characters.',
            'sequence' => 'Sequence must be unique.'
                   ];
    ...
    public function update(Request $request, $id)
    {
        ...
        $rules = [
            ...
            'sequence' => 'unique:childcategories,sequence,'.$id
                 ];
        $customs = [
            'slug.unique' => 'This slug has already been taken.',
            'slug.regex' => 'Slug Must Not Have Any Special Characters.',
            'sequence' => 'Sequence must me unique.'
        ];
        $validator = Validator::make(Input::all(), $rules, $customs);
