...
class ProductController extends Controller
{
    ...
    public function allDelete(){
        ...
        return redirect()->back()->with('message','All Products Deleted Succesfully');
    }
    //*** GET Request
    ...
    public function createPhysical()
    {
        ...
        return view('admin.product.create.physical',compact('cats','sign','shippings'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $ships = [];    
        if($request->shipping)
        foreach($request->shipping as $sh){
            $sh = $sh/$sign->value;
            array_push($ships,$sh);
        }
        $input['shipping'] = implode(',', $ships);
        ...
        $input['vat_amt'] = ($request->vat/100) * $input['price'];
        $dsc_price = ($input['price'] );
        $input['price'] = $input['price'];
        $input['dsc'] = $request->dsc;
        $input['dsc_amt'] = ($request->dsc/100) * $dsc_price;
          ...
          if (!empty($catAttrs)) {
            foreach ($catAttrs as $key => $catAttr) {
              ...
              if ($request->has("$in_name")) {
				...
				$quantities = '';
				if(@$request["$in_name"."_quantity"])
				{
					$quantities = $request["$in_name"."_quantity"];
				}
				$attrArr["$in_name"]["quantities"] = $quantities;
				
				
				$image_array = array();
				$cust_img_str = $request["$in_name"."_custom"];
				$j = 0;
				$k = 0;
				foreach($cust_img_str as $img_str)
				{
					if($img_str == 'yes')
					{					
						if(@$request->file("$in_name"."_image"))
						{
							$files = $request->file("$in_name"."_image");
							$i=0;
				...
				$attrArr["$in_name"]["images"] = $image_array;
...
//                $input['childcategory_id'] = $request->childcategory_id;
    ...
    $log .= "<br>Row No: ".$i." - Duplicate Product Code!<br>";
        ...
        //--- Redirect Section
        $msg = 'Bulk Product File Imported Successfully'.$log;
        return response()->json($msg);
    }
    ...
    public function edit($id)
    {
        ...
        elseif($data->type == 'License')
            ...
            return view('admin.product.edit.physical',compact('cats','data','sign','shippings'));
    }
             ...
             foreach ($catAttrs as $key => $catAttr) {
               ...
               if ($request->has("$in_name")) {
				...
				foreach($cust_img_str as $img_str)
				{
					...
					{
						$exists = 0;						
						$attrArr = json_decode($data->attributes, true);
						foreach ($attrArr as $attrKey => $attrVal)
						{	
							$l = 0;
							if(isset($attrVal['images']))
							{
    ...
    public function destroy($id)
    {
        ...
        // Offers
        $offers = Offer::where(['product_id' => $id])->orWhere(['free_product_id' => $id])->get();
        if($offers){
