...
class CatalogController extends Controller
{
    ...
    //   }else{
    //     return redirect('selectCountry');
    //   }

    //   $prods = $prods::with(['user' => function($query) use ($country){
    //       $query->where('country', $country->country_name);
    //   }]);
    ...
    if($request->categories){
        ...
        $prods = Product::whereIn('category_id', $categories);            
    }else{
      ...
      $prods = (new Collection(Product::filterProducts($prods)))->paginate(12);
        ...
        //     if($pro->user->country == $country->country_name){
    ...
    public function product($slug)
    {
        ...
        return view('front.product',compact('productt','curr','vendors','offers'));
    ...
    public function offer_quick($id){
        ...
        return view('load.offer_quick',compact('offer','curr'));
    }
...
//        $product->update();
    ...
    // ------------------ Rating SECTION --------------------
        ...
        {
            $ck = 0;
            $orders = Order::where('user_id','=',$request->user_id)->where('payment_status','=','completed')->get();
