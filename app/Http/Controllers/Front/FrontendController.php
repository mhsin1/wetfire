...
// -------------------------------- HOME PAGE SECTION ----------------------------------------
	...
	public function index(Request $request)
	{
        ...
        // if(Cookie::has('country')){
            ...
            return view('front.index',compact('ps','sliders','top_small_banners','products','cats','offers','ratings'));
        // }else{
        //     return redirect(route('front.selectCountry'));
        // }
	}
...
// CURRENCY SECTION
    ...
    {
        $currency = Currency::where(['country_id' => $id])->first();
        Session::put('currency', $currency->id);

        $cookie = cookie()->forever('country', $id);
        return redirect(route('front.selectState' , $id))->cookie($cookie);
    }

    public function state($id)
    {
        $cookie = cookie()->forever('state', $id);
        return redirect(route('front.selectCity' , $id))->cookie($cookie);
    }

    public function city($id)
    {
        $cookie = cookie()->forever('city', $id);
        return redirect(route('front.index'))->cookie($cookie);
    }
...
// -------------------------------- CONTACT SECTION ----------------------------------------
    ...
    public function contactemail(Request $request)
    {
        ...
        $phone = $request->phone;
        $from = $request->email;
        $msg = "Name: ".$name."<br>Email: ".$from."<br>Phone: ".$phone."<br>Message: ".$request->text;
        if($gs->is_smtp)
        {
        $data = [
            'to' => $to,
...
// -------------------------------- PRINT SECTION ENDS ----------------------------------------
    ...
    public function category_electric(){

        $cat = Category::where('slug', 'electric')->firstOrFail();
        $size = count($cat->subs);
        if($size >= 1){
            $subcat1 = $cat->subs[0]->products->take(4);
            $sub1 = $cat->subs[0]->slug;
        }else{
            $subcat1 = null;
            $sub1 = null;
        }
        ...
        if($size >= 2){
            $subcat2 = $cat->subs[1]->products->take(4);
            $sub2 = $cat->subs[1]->slug;
        }else{
            $subcat2 = null;
            $sub2 = null;
        }
        ...
        if($size >= 3){
            $subcat3 = $cat->subs[2]->products->take(4);
            $sub3 = $cat->subs[3]->slug;
        }else{
            $subcat3 = null;
            $sub3 = null;
        }
        ...
        if($size >= 4){
            $subcat4 = $cat->subs[3]->products->take(4);
            $sub4 = $cat->subs[3]->slug;
        }else{
            $subcat4 = null;
            $sub4 = null;
        }

        return view('front.category_electric' , compact('cat' , 'subcat1', 'subcat2', 'subcat3', 'subcat4', 'sub1', 'sub2', 'sub3', 'sub4'));
    }
    ...
    public function selectCountry(){
        ...
        // }else{
            ...
            return view('front.selectCountry' , compact('countries'));    
        // }
    }
    ...
    public function selectState($id){
        ...
        // }else{
            ...
            if(count($states) > 0){
                return view('front.selectState' , compact('states'));    
            }else{
                return redirect(route('front.index'));
            }
    ...
    public function selectCity($id){
        ...
        //     return redirect(route('front.index'));    
        // }else{
            $cities = City::where(['status' => 1])->where(['state_id' => $id])->take(8)->get();
            if(count($cities) > 0){
                return view('front.selectCity' , compact('cities'));    
            }else{
                return redirect(route('front.index'));
            }
        // }
