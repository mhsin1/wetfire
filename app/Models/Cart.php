...
public function addOffer($offer, $id , $price) {

    $storedItem = ['id' => $id, 'qty' => 0, 'price' => $price, 'type' => $offer->type, 't_price' => 0];
    // $qty = 0; 
    // if(isset($this->offers[$id.'_offer'])){
    //     $storedItem['qty'] = $this->offers[$id.'_offer']['qty'];
    // }

    if($offer->type == 2){
        $storedItem['qty'] = $offer->quantity;        
    }else{
        $storedItem['qty'] = $storedItem['qty'] + 1;
    }
    $storedItem['t_price'] = $price * $storedItem['qty'];
    $this->offers[$id.'_offer'] = $storedItem;
    $this->totalQty++;
}
...
// **************** REDUCING QUANTITY ENDS *******************
    ...
    public function removeOffer($id) {
        $this->totalQty -= $this->offers[$id.'_offer']['qty'];
        $this->totalPrice -= $this->offers[$id.'_offer']['price'];
        unset($this->offers[$id.'_offer']);
    }

    public function totalCount() {
        $count = 0;
        if($this->items && $this->offers){
            $count = count($this->items) + count($this->offers);
        }else if($this->items && !$this->offers){
