...
// **************************************  AJAX REQUESTS SECTION *****************************************
        ...
        $.get(getattrUrl + '?id=' + this.value + '&type=category', function(data) {
          ...
          for (var i = 0; i < data.length; i++) {
            ...
            for (var j = 0; j < data[i].options.length; j++) {
              ...
              let quantityClass = '';
              if (data[i].attribute.quantity_status == 0) {
                quantityClass = 'd-none';
              }
              let imageClass = '';
              if (data[i].attribute.image_status == 0) {
                imageClass = 'd-none';
              }
              attrHtml += `
                <div class="row mb-0 option-row">
				  ...
				  <div class="col-lg-8">
					...
					<div class="row ${quantityClass}">
						 <div class="col-2">
							&nbsp;
						 </div>
						 <div class="col-10">
							<div class="quantity-container">
								 <input type="text" class="input-field quantity-input" id="${data[i].attribute.input_name}${data[i].options[j].id}_quantity" data-name="${data[i].attribute.input_name}_quantity[]" placeholder="Quantity" value="">
							</div>
						</div>
					</div>
					...
					<div class="row ${imageClass}">
						 <div class="col-2">
						 &nbsp;
						 </div>
						 <div class="col-10">
							<div class="image-container">
								 <input type="file" class="input-field image-input" id="${data[i].attribute.input_name}${data[i].options[j].id}_image" data-name="${data[i].attribute.input_name}_image[]" data-custom="${data[i].attribute.input_name}_custom[]">
							</div>
						</div>
					</div>
  ...
  // Display Childcateogries & Attributes Ends
    ...
    $(document).on('change','#childcat',function () {
      ...
      if(link != "")
      {
        $('#subchildcat').load(link);
        $('#subchildcat').prop('disabled',false);
      }

      $.get(getattrUrl + '?id=' + this.value + '&type=subchildcategory', function(data) {
        console.log(data);
        let attrHtml = '';
        for (var i = 0; i < data.length; i++) {
          attrHtml += `
        ...
        $("#subchildcatAttributes").html(attrHtml);
        $("#subcatAttributes").html('');
        $("#childcatAttributes").html('');
      });
    });
  ...
  if ($('.attr-checkbox').length > 0) {
    $('.attr-checkbox').each(function() {
      ...
      if($(this).prop('checked') == true) {
		...
		if($("#"+$(this).attr('id')+'_quantity').length > 0)
		{
			if ($("#"+$(this).attr('id')+'_quantity').val().length > 0) {
			  // if price value is given
			  fd.append($("#"+$(this).attr('id')+'_quantity').data('name'), $("#"+$(this).attr('id')+'_quantity').val());
			} else {
			  // if price value is not given then take 0
			  fd.append($("#"+$(this).attr('id')+'_quantity').data('name'), 0);
			}
		}
		
		if($("#"+$(this).attr('id')+'_image').length > 0)
		{
			if ($("#"+$(this).attr('id')+'_image').get(0).files[0]) {
			  // if price value is given
			  fd.append($("#"+$(this).attr('id')+'_image').data('name'), $("#"+$(this).attr('id')+'_image').get(0).files[0]);
			  fd.append($("#"+$(this).attr('id')+'_image').data('custom'), 'yes');
			} else {
			  fd.append($("#"+$(this).attr('id')+'_image').data('custom'), 'no');
			}
		}
