...
//**************************** CUSTOM JS SECTION ****************************************
    ...
    $("#registerform").on('submit', function (e) {
      ...
      $this.find('button.submit-btn').prop('disabled', true);
      $this.find('.alert-info').show();
      $this.find('.alert-info p').html("Sending Request");
      $.ajax({
        method: "POST",
        url: $(this).prop('action'),
        data: new FormData(this),
        ...
        success: function (data) {
          ...
          if (data == 2) {
            window.location = mainurl + '/user/verify-page';
          }
          if (data == 1) {
...
// Offer Quick View Section

$(document).on('click', '.offer-quick-a', function(){
  var $this = $("#offerquickview");
  $this.find('.modal-header').hide();
  $this.find('.modal-body').hide();
  $this.find('.modal-content').css('border','none');
    $('.submit-loader').show();
    $(".offer-quick-view-modal").load($(this).data('href'),function(response, status, xhr){
      if(status == "success"){
        $('.submit-loader').hide();
        $this.find('.modal-header').show();
        $this.find('.modal-body').show();
...
// Wishlist Section
    ...
    $(document).on('click', '.add-to-wish', function(){
      ...
      $.ajax({
        ...
        statusCode: {
           401: function() {
             window.location.href= $('#user_login_url').val();
         }
       },
       success:function(data){
          if(data[0] == 1) {
            ...
            $('.wish_'+id).addClass('fa-heart');          
            $('.wish_'+id).removeClass('fa-heart-o');
            }
          else {
...
// Cart Section
    ...
    $(document).on('click', '.add-offer-to-cart', function(){
  ...
  $(document).on('click', '.cart-offer-remove', function(){
    ...
    $('.'+$selector).hide();
      $.get( $(this).data('href') , function( data ) {
          if(data == 0) {
             ...
             $('.discount').html($("#c-val").val()+parseFloat(parseFloat(data[5]) + parseFloat($('#coupon_val').val())).toFixed(2));              
             $('.coupon-total').val(data[0]);
             $("#tax-total").html(data[25]);
             $("#cart-paisa").html(data[9]);
             $(".main-total").html($("#c-val").val()+parseFloat(parseFloat(data[3].slice($("#c-sign-length").val())) - parseFloat(data[5])).toFixed(2));
             $(".main-total").html($("#c-val").val()+parseFloat(parseFloat(data[3].slice($("#c-sign-length").val()) - $('#coupon_val').val()) - parseFloat(data[5])).toFixed(2));
             $("#cart-items").load(mainurl+'/carts/view');
            }
      });
  });
...
// Adding Muliple Quantity Starts
    ...
    $(document).on('click', '.product-size .siz-list .box', function () {
        ...
        var parent = $(this).parent();
         ...
         $('#dynamic_price').html(sign+total);
         $('#dynamic_price').attr('value',total);
         }
         else {
         $('#sizeprice').html(total+sign);
         $('#dynamic_price').html(total+sign);
         $('#dynamic_price').attr('value',total);
         }
...
$(document).on("click", ".addcart_wishlist" , function(){
      var pid = this.id;
      var qty = $('#'+pid+'_input').val();
            ...
            $.ajax({
                    ...
                    success:function(data){      
                      if(data == 0)
                        ...
                        {
                        $(".discount").html($("#c-val").val()+parseFloat(parseFloat($("#coupon_val").val())+parseFloat($("#d-val").val().slice($("#c-sign-length").val()))).toFixed(2));
                        $(".cart-total").html(data[0]);
                        $(".main-total").html($("#c-val").val()+parseFloat(parseFloat(data[3].slice($("#c-sign-length").val()) - $('#coupon_val').val()) - $('#d-val').val().slice($("#c-sign-length").val())).toFixed(2));
            ...
            $.ajax({
                    ...
                    success:function(data){
                        ...
                        {                        
                            ...
                            $("#coupon_form").toggle();
                            $(".main-total").html($("#c-val").val()+parseFloat(parseFloat(data[0].slice($("#c-sign-length").val())) - parseFloat($('#d-val').val().slice($("#c-sign-length").val()))).toFixed(2));
                            $(".discount").html($("#c-val").val()+parseFloat(parseFloat(data[2]) + parseFloat($('#d-val').val().slice($("#c-sign-length").val()))).toFixed(2));
                            $("#coupon_val").val(data[4].slice($("#c-sign-length").val()));
